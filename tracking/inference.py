...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # the domain of pacman and the ghosts can take on any position in the grid
    variableDomainsDict[PAC] = variableDomainsDict[GHOST0] = variableDomainsDict[
        GHOST1
    ] = list(itertools.product(range(X_RANGE), range(Y_RANGE)))
    # the domain of observations are non negative manhattan distances of pacman to ghosts +- noise
    variableDomainsDict[OBS0] = variableDomainsDict[OBS1] = list(
        range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)
    )
    "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # when gost is in the jail, return posibility 1
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        numPInEachPos = int(self.numParticles / len(self.legalPositions))
        numRemainingP = self.numParticles % len(self.legalPositions)
        for legalPos in self.legalPositions:
            self.particles.extend([legalPos] * numPInEachPos)
        if numRemainingP > 0:
            for i in range(numRemainingP):
    ...
    def getBeliefDistribution(self):
        ...
        for legalPos in self.legalPositions:
            posCount = self.particles.count(legalPos)
            posProb = posCount / self.numParticles
            beliefs[legalPos] = posProb
        return beliefs
        "*** END YOUR CODE HERE ***"
